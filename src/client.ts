...
import { AccountBalance, FormattedResponse, MessageSendingResult, Prices } from './types'
...
export class Client {
  ...
  constructor(options: {
    ...
    this.#baseUrl = 'https://gatewayapi.com'
    this.#token = options.token
  }

  getAccountBalance = async (): Promise<FormattedResponse<AccountBalance>> => {
    const res = await fetch(this.#baseUrl + '/rest/me')
    ...
    if (!res.ok) {
      ...
      return {
        error: {
          code: res.status,
          reason: contentType?.includes('application/json')
            ? await res.json()
            : await res.text()
        }
      }
  ...
  getPrices = async <Format extends 'json' | 'csv' | 'xlsx'>(options: {
    format: Format
  }): Promise<FormattedResponse<Format extends 'json' ? Prices : ArrayBuffer>> => {
    const res = await fetch(this.#baseUrl + '/api/prices/list/sms/' + options.format)
  ...
  }): Promise<FormattedResponse<MessageSendingResult>> => {
    const res = await fetch('https://gatewayapi.com/rest/mtsms', {
      method: 'POST',
      headers: {
        Authorization: `Basic ${encodeBase64(this.#token + ':')}`,
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({
        callback_url: options.callbackUrl,
        class: options.class?.toLowerCase(),
